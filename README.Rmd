...
This gives a lot of output, but we only really care about the `$root` value, which is `r intersection_funs$root %>% round(3)`. And sure enough, it calculated the correct intersection!

```{r supply-demand-intersection-x}
ggplot(mapping = aes(x = x, y = y)) + 
  geom_path(data = supply, color = "#0073D9", size = 1) + 
  geom_path(data = demand, color = "#FF4036", size = 1) + 
...
To get the horizontal intersection, we just have to find where the vertical intersection (`r intersection_funs$root %>% round(3)`) shows up in the demand function. We calculate this by plugging the intersection into the `fun_demand()` function:

```{r supply-demand-intersection-xy}
...
```{r supply-demand-intersection-simple}
